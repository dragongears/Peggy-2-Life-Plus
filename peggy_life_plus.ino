...
Modified by Art Dahm (art@dahm.com)
- Hold down "Any" button when entering edit mode to clear display
...
unsigned int history[HISTORY_GENERATIONS];
...
/////////////////////////////////////////////////////////////////////////
// Used by boringness detector

unsigned int pattern_repeat(void)
{
	unsigned int repeat = 0;

	for (int gens = HISTORY_GENERATIONS/2; gens >= 1; gens--)
	{
		int notsame = 0;

		for (int gen = gens-1; gen >= 0; gen--)
			if (history[gen] != history[gen+gens])
				notsame++;
...
void push_generation(unsigned long total) {
	for (int x = 1; x < HISTORY_GENERATIONS; x++) {
...
void loop()											 // run over and over again
{
    ...
    // Boringness detector
		push_generation(get_total(current_generation));
		if (pattern_repeat())
			generations++;
		else
			generations = 0;
		...
		{
			current_generation[11] = rand() & 0xFF00;
			current_generation[12] = rand() & 0xFF00;
			current_generation[13] = rand() & 0xFF00;
			generations = 0;
		}
